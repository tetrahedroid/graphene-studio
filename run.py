...
GMX = os.environ.get("GMX", "gmx")  # 環境変数GMXが設定されていたら、それをパスとみなす。
...
def run(mdp="continue.mdp", job_name="4P2005", N_thread=8, job_range=(1,2), index_file=None):
    """
    初期配置からの最初の実行
    ...
    for num in range(*job_range):
        ...
        # if num == 1:
        #     command = f"{GMX} grompp -maxwarn 1 -f {mdp} -p {top} -c {gro} -o {tpr} > {num:05d}.grompp.log 2>&1"
        # else:
        #     command = f"{GMX} grompp -maxwarn 1 -f {mdp} -p {top} -c {previous_tpr} -t {previous_cpt} -o {tpr} > {num:05d}.grompp.log 2>&1"
...
if __name__=="__main__":
    ...
    if len(sys.argv) not in (2,3,4):
        ...
        logger.error("\tjob#      Either empty, single number, or two numbers specifying the first and last job number.")
        sys.exit(1)
